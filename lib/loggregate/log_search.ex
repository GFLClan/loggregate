...
defmodule Loggregate.LogSearch do
  ...
  def build_es_query(search_string, {start_date, end_date}) do
    try do
      ...
      {:ok, %{bool: %{must: compile_es_query(ast), filter: [%{range: %{timestamp: %{gte: start_date, lte: end_date}}}]}}}
    rescue
      _e -> {:error, nil}
    end
  end
  ...
  def build_search_predicate(search_string) do
    try do
      {ast, []} = DreaeQL.parse(search_string)
      {:ok, compile_predicate(ast)}
    rescue
      _e -> {:error, nil}
    end
  end

  def compile_predicate(ast) do
    compile_predicate(ast, fn (%ParsedLogEntry{} = _entry) -> true end)
  end
